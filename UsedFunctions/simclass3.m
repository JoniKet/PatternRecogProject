function [Mean_accuracy, Variance,p1,m1,pp,mm,id,maxacc]=simclass3(data,v,c, measure, p, m, N,rn,pl)
%Inputs:
        ...
        for i = 1:length(p)  
            ...
            for k = 1 : N 
                ...
                if w_opt == 0  % No weight optimization
                    [fitness(k), class, Simil] = calcfitness2(data(data_ind,:), ideals(:,:,k), y, w);
                    if fitness(k)>maxacc
                        pp=p(i);
                        mm=m(j);
                        id=ideals(:,:,k);
